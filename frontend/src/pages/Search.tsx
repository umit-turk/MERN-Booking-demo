...
const Search = () => {
    ...
    const handleHotelTypesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedHotelTypes((hotelTypes) => event.target.checked ? [...hotelTypes, hotelType] : hotelTypes.filter((type) => type !== hotelType))
    }
    const handleFacilitiesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const facility = event.target.value;
        setSelectedFacilities((facilities) => event.target.checked ? [...facilities, facility] : facilities.filter((type) => type !== facility))
    }
        ...
        <div className='grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5'>
            <div className='rounded-lg border border-slate-300 p-5 h-fit sticky top-10'>
                <div className='space-y-5'>
                    <h3 className='text-lg font-semibold border-b border border-slate-300 pb-5'>
                        Filter by:
                    </h3>
